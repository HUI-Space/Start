...
namespace Start
{
    ...
    public class RecyclablePoolBucket
    {
        ...
        private readonly ConcurrentQueue<IRecycle> _unused = new ConcurrentQueue<IRecycle>();
        ...
        {
            return new RecyclablePoolStats(
                _type,
                Volatile.Read(ref _unusedCount),
                Volatile.Read(ref _usingCount),
                Volatile.Read(ref _acquireCount),
                Volatile.Read(ref _releaseCount),
                Volatile.Read(ref _addCount),
                Volatile.Read(ref _removeCount),
                Volatile.Read(ref _totalCreateCount),
                Volatile.Read(ref _peakUsingCount),
                DateTime.Now
            );
        }
        ...
        /// <returns></returns>
        public T Acquire<T>() where T : class, IRecycle, new()
        {
            if (_unused.TryDequeue(out IRecycle item))
            {
        ...
        public void Recycle(IRecycle recycle)
        {
            if (recycle == null)
            {
            ...
            recycle.Recycle();
#if DEBUG
            if (_unused.Contains(recycle))
            {
                throw new Exception("The recycle has been released.");
            }
#endif
            _unused.Enqueue(recycle);
        ...
        public void Add(int count)
        {
            ...
            {
                _unused.Enqueue(Activator.CreateInstance(_type) as IRecycle);
