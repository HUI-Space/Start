    ...
    public class BuildLocalBuiltInManifestTask:IBuildTask
    {
        public void Run(IResourceBuilder builder)
        {
            ...
            foreach (string asset in builtInAssets)
            {
                ...
                {
                    Log.Error($"没有找到模块资源包信息,资源名称：{asset}");
                    continue;
                }
            ...
            string builtInPath = ResourceHelper.GetResourcePath(EResourceType.BuiltIn);
            //创建基础资源文件夹
            ...
            FileUtility.WriteAllText(builtInResourceManifestPath,content);
        }
    }
}