        ...
        public void Run(IResourceBuilder builder)
        {
            ...
            foreach (ResourceGroup group in groups)
            {
                foreach (ResourceGroupItem groupItem in group.GroupItems)
                {
                    ...
                    string[] assets = EditorUtility.FindAssets(SearchType.All, groupItem.Path);
            ...
            foreach (KeyValuePair<string, MainResource> item in allMainResource)
            {
                ...
                List<string> allDependencies = EditorUtility.GetAllDependencies(mainResourcePath);
            ...
            void Print()
            {
                ...
                foreach (KeyValuePair<string,MainResource> item in allMainResource)
                {
                    ...
                    Log.Info($"--------------------->{nameof(MainResource)}<---------------------");
                    Log.Info($"{nameof(MainResource.CollectorType)}:        {mainResource.CollectorType}" );
                    Log.Info($"{nameof(MainResource.AddressType)}:          {mainResource.AddressType}" );
                    Log.Info($"{nameof(MainResource.PackType)}:             {mainResource.PackType}" );
                    Log.Info($"{nameof(Resource.Path)}:                 {mainResourcePath}");
                    Log.Info($"{nameof(MainResource.Tags)}:                 {mainResource.Tags}"); 
                    Log.Info($"Dependencies Count:   {mainResource.Resource.AllDependencies.Count}");   
                    foreach (string path in mainResource.Resource.AllDependencies)
                    {
                        Log.Info($"Dependencies:         {path}");   
                    }
                ...
                foreach (KeyValuePair<string, Resource> item in allResource)
                {
                    ...
                    Log.Info($"--------------------->{nameof(Resource)}<---------------------");
                    Log.Info($"{nameof(Resource.Path)}:                 {resource.Path}");
                    Log.Info($"{nameof(Resource.GUID)}:                 {resource.GUID}");
                    Log.Info($"{nameof(Resource.Type.Name)}:                 {resource.Type.Name}");
                    Log.Info($"{nameof(Resource.Extension)}:            {resource.Extension}");
                    Log.Info(string.Empty);
                }
                ...
                {
                    Log.Info($"--------------------->{nameof(AssetBundleBuild)}<---------------------");
                    Log.Info($"{nameof(AssetBundleBuild.assetBundleName)}:      {build.assetBundleName}");
                    Log.Info($"{nameof(AssetBundleBuild.assetBundleVariant)}:   {string.Empty}");
                    foreach (string asset in build.assetNames)
                    {
                        Log.Info($"{nameof(AssetBundleBuild.assetNames)}:           {asset}");
                    }
                    Log.Info(string.Empty);
                }
            }
