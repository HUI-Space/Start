...
namespace Start.Framework
{
    public class ReportLog : ILogHelper,IReference
    {
        ...
        {
            FileUtility.WriteAllText(name, path, _stringBuilder.ToString());
        }

        private void LogInternal(ELogType logType, string message, object[] args)
        {
            _stringBuilder.AppendFormat("[{0:yyyy-MM-dd HH:mm:ss.fff}][{1}] ", DateTime.UtcNow.ToLocalTime(), logType.ToString());
