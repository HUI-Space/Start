...
namespace Start.Runtime
{
    ...
    public class ProcedureResourceUpdate : ProcedureBase
    {
        ...
        {
            RuntimeMessage.AddListener((int)EMessageId.ConfirmUpdateResource, OnConfirmUpdateResource);
            ...
            RuntimeMessage.SendMessage((int)EMessageId.DisplayUpdateResourceInfo, genericData);
            return base.OnEnter();
        }

        protected override Task OnExit()
        {
            RuntimeMessage.RemoveListener((int)EMessageId.ConfirmUpdateResource, OnConfirmUpdateResource);
            DownloadManager.Instance.DownloadEventHandler -= DownloadEventHandler;
        ...
        private void DownloadEventHandler(DownloadEvent downloadEvent)
        {
            ...
            {
                Log.Info($"DownloadEventHandler Done: {downloadEvent.DownloadUri} ");
        ...
        private async void DownloadComplete()
        {
            ...
            await ChangeState<ProcedureLaunch>();
        }
        ...
        private void UpdateLocalManifest()
        {
            ...
            foreach (ResourceInfo resourceInfo in localManifest.Resources)
            {
                if (resourceInfos.TryGetValue(resourceInfo.Name,out ResourceInfo resource))
                {
                    ...
                    {
                        Log.Error($"已存在资源{resourceInfo.Name}");
                    }
                }
                else
                {
            ...
            Log.Info($"更新本地manifest成功");
        }

        private void UpdateLocalResourceVersion()
        {
            Log.Info($"更新本地资源版本:{GameConfig.RemoteResourceVersion}");
            FileUtility.WriteAllText(ResourceConfig.LocalResourceVersionPath,GameConfig.RemoteResourceVersion);
        }
