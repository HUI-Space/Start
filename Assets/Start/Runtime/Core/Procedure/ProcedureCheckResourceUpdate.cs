...
namespace Start.Runtime
{
    ...
    public class ProcedureCheckResourceUpdate : ProcedureBase
    {
        ...
        {
            RuntimeMessage.AddListener((int)EMessageId.ReGetRemoteMandatoryManifest, OnReGetRemoteMandatoryManifest);
            RuntimeMessage.AddListener((int)EMessageId.ReGetRemoteOptionalManifest, OnReGetRemoteOptionalManifest);
            await GetRemoteMandatoryManifest();
        }

        protected override Task OnExit()
        {
            RuntimeMessage.RemoveListener((int)EMessageId.ReGetRemoteMandatoryManifest, OnReGetRemoteMandatoryManifest);
            RuntimeMessage.RemoveListener((int)EMessageId.ReGetRemoteOptionalManifest, OnReGetRemoteOptionalManifest);
            return base.OnExit();
        }
        ...
        private async Task GetRemoteMandatoryManifest()
        {
            ...
            {
                Log.Error("获取远程资源清单失败:{0}", error);
                IGenericData genericData = GenericData<long, string>.Create(responseCode, error);
                RuntimeMessage.SendMessage((int)EMessageId.GetRemoteMandatoryManifestFailure, genericData);
        ...
        private async Task GetRemoteOptionalManifest()
        {
            ...
            else
            {
                Log.Error("获取远程资源清单失败:{0}", error);
                IGenericData genericData = GenericData<long, string>.Create(responseCode, error);
                RuntimeMessage.SendMessage((int)EMessageId.GetRemoteOptionalManifestFailure, genericData);
                ReferencePool.Release(genericData);
            }
        }
        ...
        private async void CheckResourceUpdate()
        {
                ...
                foreach (var item in remoteOptionalResourceManifests.Values)
                {
                    foreach (var remoteResourceInfo in item.Resources)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Log.Error($"资源名称相同,但MD5不同,资源名称：{remoteResourceInfo.Name}" );
                            }
                        }
                    }
                }
            ...
            await ChangeState<ProcedureResourceUpdate>();
        }
    }
}