...
namespace Start.Runtime
{
    ...
    public class ProcedureCheckResourceVersion : ProcedureBase
    {
        ...
        {
            RuntimeMessage.AddListener((int)EMessageId.ReGetRemoteResourceVersion, OnReGetRemoteResourceVersion);
            //判断是否开启热更
        ...
        {
            RuntimeMessage.RemoveListener((int)EMessageId.ReGetRemoteResourceVersion, OnReGetRemoteResourceVersion);
            return base.OnExit();
        }
        ...
        private async Task GetRemoteResourceVersion()
        {
            ...
            {
                Log.Fatal("获取远程版本号错误:" + response.Error);
                IGenericData genericData = GenericData<long, string>.Create(responseCode, error);
                RuntimeMessage.SendMessage((int)EMessageId.GetRemoteResourceVersionFailure, genericData);
        ...
        private async void CheckResourceVersion(string LocalResourceVersionPath,byte[] data)
        {
            ...
            await ChangeState<ProcedureResourceVerify>();;
