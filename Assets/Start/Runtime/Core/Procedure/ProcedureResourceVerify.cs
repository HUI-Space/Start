...
namespace Start.Runtime
{
    ...
    public class ProcedureResourceVerify : ProcedureBase
    {
        ...
        {
            RuntimeMessage.AddListener((int)EMessageId.ReGetBuiltInResource, OnReGetBuiltInResource);
            //判断本地依赖配置是否存在
            if (!File.Exists(ResourceConfig.LocalManifestPath))
            {
                Log.Error("获取本地依赖信息失败");
        ...
        {
            RuntimeMessage.RemoveListener((int)EMessageId.ReGetBuiltInResource, OnReGetBuiltInResource);
            return base.OnExit();
        }
        ...
        private async Task VerifyLocalResource(Manifest localManifest,byte[] builtIn)
        {
            ...
            foreach (ResourceInfo resourceInfo in localManifest.Resources)
            {
                if (resourceInfo.ResourceType == EResourceType.BuiltIn)
                {
                    ...
                    {
                        Log.Error("内置资源与依赖配置错误! 资源文件MD5校验失败:{0}", resourceInfo.Name);//通常是修改本地文件
                        needInitialize = true;
                        break;
                    }
                }
                else
                {
                    ...
                    {
                        Log.Error("资源文件MD5校验失败:{0}", resourceInfo.Name);
                        break;
                    }
                }
                await Task.Delay(1);
            ...
            await ChangeState<ProcedureCheckResourceUpdate>();
        }
        ...
        private async Task GetBuiltInResource()
        {
            ...
            {
                Log.Error("获取内嵌资源失败:{0}", response.Error);
                IGenericData genericData = GenericData<long, string>.Create(responseCode, error);
                RuntimeMessage.SendMessage((int)EMessageId.GetBuiltInResourceFailure, genericData);
