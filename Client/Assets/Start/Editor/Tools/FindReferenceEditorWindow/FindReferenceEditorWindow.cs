...
public class FindReferenceEditorWindow : EditorWindow
{
    public static void OpenEditorWindow()
    {
        var window = GetWindow<FindReferenceEditorWindow>("资源引用查找");
    ...
    private class ReferenceResult
    {
        ...
        public ReferenceResult(string sourceName, string sourcePath, string[] refrences)
        {
    ...
    private List<ReferenceResult> mRefrenceResult = new List<ReferenceResult>();
    ...
    private void DrawFilter()
    {
        ...
        {
            int nCounter = 0;
            foreach (ReferenceResult rRefrenceResult in this.mRefrenceResult)
            {
    ...
    private void DrawDeleteNoReference()
    {
        if (GUILayout.Button("删除无引用的资源", GUILayout.Width(400f)))
        {
            ...
            {
                ReferenceResult rReferenceResult = this.mRefrenceResult[i];
                if (rReferenceResult.Skip)
                {
                    continue;
                }
                else
                {
                    if (rReferenceResult.Refrences.Length == 0)
                    {
                        results.Add(rReferenceResult);
                    }
                }
            }
    ...
    private void DrawContent()
    {
            ...
            for (int i = 0; i < this.mRefrenceResult.Count; i++)
            {
                ...
                else
                {
                    if (nIndex >= nStartIndex)
                    {
                        ...
                        if (rReferenceResult.Refrences.Length == 0)
                        {
                            DrawRefrenceResultNoRefrences(nIndex, rReferenceResult);
                        }
                        else
                        {
                            DrawRefrenceResult(nIndex, rReferenceResult);
                        }
        ...
        void DrawByDuplicate()
        {
            ...
            {
                ReferenceResult rReferenceResult = this.mRefrenceResult[i];
                if (rReferenceResult.DuplicateID != this.mPageIndex)
                {
                    continue;
                }
        ...
        void DrawRefrenceResultNoRefrences(int index, ReferenceResult result)
        {
        ...
        void DrawRefrenceResult(int index, ReferenceResult result)
        {
            rect = EditorGUILayout.BeginHorizontal();
    ...
    private void FindRefrences()
    {
        ...
        foreach (string rAssetPath in rAssetPaths)
        {
            ...
            this.mRefrenceResult.Add(new ReferenceResult(Path.GetFileNameWithoutExtension(rAssetPath), rAssetPath, rRefrences.ToArray()));
        }
    ...
    private void BeginDetectionDuplicate()
    {
        ...
        foreach (KeyValuePair<string, HashSet<string>> rMD5ToPathsPair in rMD5ToPathsDictionary)
        {
            ...
            foreach (string rAssetPath in rAssetPaths)
            {
                ...
                var result = new ReferenceResult(Path.GetFileNameWithoutExtension(rAssetPath), rAssetPath, rRefrences.ToArray());
