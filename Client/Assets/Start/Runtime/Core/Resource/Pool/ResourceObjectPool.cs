...
namespace Start
{
    public class ResourceObjectPool<TObject, TTarget> : IResourceObjectPool<TObject, TTarget>
        where TObject : ResourceObjectBase<TTarget>, new() 
        where TTarget : class
    {
        ...
        public void Initialize(int capacity = Int32.MaxValue, bool allowMultiSpawn = false, float autoReleaseInterval = float.MaxValue, float expireTime = float.MaxValue, int priority = 0)
        {
            Capacity = capacity;
        ...
        public void DeInitialize()
        {
            foreach (KeyValuePair<TTarget, Object> objectInMap in _objectMap)
            {
                ...
                RecyclablePool.Recycle(objectInMap.Value);
            }
            _objects.Clear();
            _objectMap.Clear();
        ...
        private bool ReleaseObject(TTarget target)
        {
            ...
            RecyclablePool.Recycle(internalObject);
            return true;
        }
        ...
        private sealed class Object : IRecycle
        {
            ...
            {
                var pooled = RecyclablePool.Acquire<Object>();
                pooled.Target = obj;
            ...
            public void DeInitialize(bool isShutdown)
            {
                ...
                RecyclablePool.Recycle(Target);
            }

            public void Recycle() => (Target, SpawnCount) = (default, default);
        }
    }
}