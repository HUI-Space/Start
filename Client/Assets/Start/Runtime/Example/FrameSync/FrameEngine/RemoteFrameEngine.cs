...
namespace Start
{
    public class RemoteFrameEngine : FrameEngineBase
    {
        /// <summary>
        /// 预测帧
        /// </summary>
        ...
        public MatchEntity AuthorityMatchEntity;
        /// <summary>
        /// 预测实体
        /// </summary>
        ...
        protected override void LogicUpdate()
        {
            ...
            while (true)
            {
                ...
                //更新表现层
                RenderController.Instance.LogicUpdate(PredictionMatchEntity);
            }
            ...
            while (_authorityFrameQueue.Count > 0)
            {
                ...
                if (predictionFrame.Equals(authorityFrame))
                {
                    ...
                    MatchController.Instance.UnSpawnMatchEntity(lastAuthorityMatchEntity);
                }
                else
                {
                    //权威帧和预测帧不一致 回滚
                    MatchController.Instance.LogicUpdate(AuthorityMatchEntity,authorityFrame,_timeCounter.FrameTime(authorityFrame.AuthorityFrame));
                    
                    //预测帧队列出队回收
                    while (_predictionMatchEntityQueue.Count > 0)
                    {
                        MatchEntity predictionMatchEntity = _predictionMatchEntityQueue.Dequeue();
                        MatchController.Instance.UnSpawnMatchEntity(predictionMatchEntity);
                    }
                    _predictionFrameQueue.Clear();
                    
                    //将权威帧数据拷贝给预测帧，然后再回收
                    MatchEntity lastPredictionMatchEntity = PredictionMatchEntity;
                    MatchEntity spawnMatchEntity = MatchController.Instance.SpawnMatchEntity(AuthorityMatchEntity);
                    PredictionMatchEntity = spawnMatchEntity;
                    MatchController.Instance.UnSpawnMatchEntity(lastPredictionMatchEntity);
        ...
        {
            MatchController.Instance.LogicUpdate(PredictionMatchEntity,predictionFrame,_timeCounter.FrameTime(predictionFrame.AuthorityFrame));
            MatchEntity matchEntity = MatchController.Instance.SpawnMatchEntity(PredictionMatchEntity);
